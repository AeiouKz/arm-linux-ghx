...
typedef struct BitBmp{
	...
	unsigned char buf[800*480*3];//注意 为什么需要用无符号表示
}bitBmp;
...
bitBmp* create_bitBmp_file(const char *filename)
{
	...
	ret = read(pic_fd,bmp->buf,800*480*3);
	if(ret < 0)
	{
...
//修改处
int draw_bmp(LcdDevice* lcd,int width,int high,bitBmp* bmp)
{
	printf("%d-%d,%d,",bmp->bw,bmp->bh,bmp->pertype);
	
	//显示
	int i,j;
	unsigned int a,r,g,b,color;
	unsigned char *p = bmp->buf;
